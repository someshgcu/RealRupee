...
import { Search, ChevronRight, Shield, Clock, Star, ArrowRight } from "lucide-react";
...
const Index = () => {
  ...
  const recent = [...properties].sort((a, b) => b.postedDate.localeCompare(a.postedDate));
  ...
  const CarouselSection = ({ title, items, icon: Icon, scrollRef }: {
    title: string; items: typeof properties; icon: typeof Shield; scrollRef: React.RefObject<HTMLDivElement>;
  }) => (
  ...
  return (
    <div>
      ...
      <section className="relative flex min-h-[420px] items-center justify-center overflow-hidden">
        <img src={heroBg} alt="Real estate cityscape" className="absolute inset-0 h-full w-full object-cover" />
        ...
        <div className="relative z-10 mx-auto max-w-2xl px-4 py-16 text-center">
          ...
          <p className="mb-8 text-base text-primary-foreground/80 md:text-lg">
            Trusted listings with complete legal verification, group buying, and auction support.
          </p>
          <div className="flex items-center overflow-hidden rounded-md bg-card shadow-lg">
            ...
            />
            <button className="mr-1.5 rounded-md bg-accent px-6 py-2.5 text-sm font-medium text-accent-foreground transition-colors hover:opacity-90">
              Search
            </button>
          </div>
        </div>
      ...
      {/* Carousels */}
      <CarouselSection title="Legally Validated" items={validated} icon={Shield} scrollRef={scrollRef1} />
      <div className="border-t" />
      <CarouselSection title="Recently Added" items={recent} icon={Clock} scrollRef={scrollRef2} />
      <div className="border-t" />
      <CarouselSection title="Featured Properties" items={featured} icon={Star} scrollRef={scrollRef3} />
          ...
          <section className="py-8">
            <div className="container mx-auto px-4">
              ...
              <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
                {userProperties.slice(0, 3).map((p) => (
