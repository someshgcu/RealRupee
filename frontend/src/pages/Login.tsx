...
const Login = () => {
  ...
  const [fullName, setFullName] = useState(""); // For signup sync if needed
  ...
  const handleSendOtp = async (e: FormEvent) => {
    ...
    if (error) {
      ...
      toast({
        title: "OTP Sent",
        description: `We've sent a 6-digit code to ${value}.`,
      });
    }
  };
  ...
  const handleVerifyOtp = async (otpValue: string) => {
    ...
    const { error } = await verifyOtp(value, otpValue, type);
    setLoading(false);
    ...
    return (
      <div className="flex min-h-[calc(100vh-4rem)] items-center justify-center px-4 py-12">
        <div className="w-full max-w-md rounded-lg border bg-card p-8 shadow-sm">
          ...
          <h1 className="mb-1 text-2xl font-bold text-foreground">Verify OTP</h1>
          <p className="mb-6 text-sm text-muted-foreground">
            Enter the 6-digit code sent to {tab === "otp" ? phone : email}
          </p>
          ...
          <div className="space-y-6">
            ...
            <button
              ...
              className="w-full rounded-md bg-primary py-2.5 text-sm font-medium text-primary-foreground transition-all hover:opacity-90 disabled:opacity-50 flex items-center justify-center"
            >
